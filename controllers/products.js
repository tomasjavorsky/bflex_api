...
const getEachCategoryImages = (req,res,db) => {
  ...
  db.raw("select distinct on (category_id) category_id, category_name, category_order, product_image from product_data full outer join product_categories on product_data.product_category = product_categories.category_name order by category_order")
    .then(data => res.json(data))
